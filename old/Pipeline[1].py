...
def filterData(data):
                    ...
                    if 'entries' in data[channelID][videoID]:
                        ...
                        if not(None in a):
                            try:
                                ...
                                pass
        maxDate = (max(maxDate) + (time.mktime((datetime.datetime(year=2017, month=1, day=1)).timetuple()) - time.mktime((datetime.datetime(year=2017, month=2, day=11)).timetuple())))
                    ...
                    if 'entries' in data[channelID][videoID]:
                        ...
                        for i in range(len(data[channelID][videoID]['entries'])):
                            ...
                            if not(None in a) and ((time.mktime((datetime.datetime(year=int(uploadDate[:4]), month=int(uploadDate[4:6]), day=int(uploadDate[6:]))).timetuple())) <= maxDate):
                                try:
                                    ...
                                    temp['resolution'] = int(str(data[channelID][videoID]['entries'][i]['height']))
                                    temp['age_limit'] = int(data[channelID][videoID]['entries'][i]['age_limit'])
                                    ...
                                    temp['tags'] = len(data[channelID][videoID]['entries'][i]['tags'])
                        ...
                        if not(None in a) and ((time.mktime((datetime.datetime(year=int(uploadDate[:4]), month=int(uploadDate[4:6]), day=int(uploadDate[6:]))).timetuple())) <= maxDate):
                            try:
                                ...
                                temp['resolution'] = int(str(data[channelID][videoID]['height']))
                                temp['age_limit'] = int(data[channelID][videoID]['age_limit'])
                                ...
                                temp['tags'] = len(data[channelID][videoID]['tags'])
    ...
    trainSigmoidNorm(data=filtered, labels=['video_view_count', 'tags', 'description_symbol', 'description_text', 'description_lower', 'description_upper', 'title_symbol', 'title_text', 'title_lower', 'title_upper', 'categories', 'URLs', 'numberURLs', 'description_length', 'title_length', 'duration'])
    discretize_train(data=filtered, labels=['video_view_count', 'tags', 'resolution', 'description_symbol', 'description_text', 'description_lower', 'description_upper', 'title_symbol', 'title_text', 'title_lower', 'title_upper', 'categories', 'URLs', 'numberURLs', 'description_length', 'title_length', 'duration'])
...
def classify(data):
    ...
    try:
        accuracies['RandomForest'] = cross_val_score(BaggingClassifier(RandomForestClassifier(n_estimators=100)), data, labels, cv=10)
    except:
